...
class ImportDraquexFieldSpawnsV4 extends Command
{
    protected $signature = 'dq10:import-draquex-field-v4
                            {list_url : 例 https://draquex.com/monster/field/1-augu.php}
                            {--dry-run : DBに保存しない}
                            {--map= : 特定マップ名だけに絞る}';

    protected $description = 'Draquex の一覧ページから各マップページを巡回して monster_map_spawns に投入する(area/noteはjson)';

    public function handle(): int
    {
        $this->warn('ImportDraquexFieldSpawnsV4 running');
        ...
        foreach ($mapLinks as $mapLink) {
            ...
            foreach ($spawnRows as $row) {
                ...
                $grouped[$key]['notes'] = array_merge($grouped[$key]['notes'], $note);
            }
            ...
            foreach ($grouped as $item) {
                ...
                if ($dryRun) {
                    ...
                    DB::table('monster_map_spawns')->updateOrInsert(
                        [
    ...
    private function extractMapLinksFromListPage(string $html, string $baseUrl): array
    {
        ...
        foreach ($xpath->query('//a[@href]') as $a) {
            ...
            if (
                ! str_contains($href, '/monster/field/a-')
                && ! preg_match('#^a-\d+\.php$#', $href)
            ) {
                continue;
            }
